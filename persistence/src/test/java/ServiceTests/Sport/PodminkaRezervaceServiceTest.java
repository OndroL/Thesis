...
public class PodminkaRezervaceServiceTest {
    ...
    void testFindByObjekt_Success() throws FinderException {
        ...
        when(repository.findByObjekt(eq(objektId), anyInt(), anyInt())).thenReturn(expectedEntities);

        List<PodminkaRezervaceEntity> result = service.findByObjekt(objektId, 0, 10);

        assertNotNull(result);
        assertEquals(1, result.size());
        verify(repository, times(1)).findByObjekt(eq(objektId), anyInt(), anyInt());
    }
    ...
    void testFindByObjekt_Failure() {
        ...
        when(repository.findByObjekt(eq(objektId), anyInt(), anyInt())).thenThrow(new RuntimeException("Database error"));
