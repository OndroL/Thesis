...
public class SportServiceTest {
    ...
    void testFindByCategory_Success() throws FinderException {
        ...
        when(repository.findByCategory(eq(categoryId), any())).thenReturn(expectedEntities);

        List<SportEntity> result = service.findByCategory(categoryId, offset, count);

        assertNotNull(result);
        assertEquals(1, result.size());
        verify(repository, times(1)).findByCategory(eq(categoryId), any());
    }
    ...
    void testFindByCategory_Failure() {
        ...
        when(repository.findByCategory(eq(categoryId), any())).thenThrow(new RuntimeException("Database error"));
    ...
    void testFindByZbozi_Success() throws FinderException {
        ...
        when(repository.findByZbozi(eq(zboziId), any())).thenReturn(expectedEntities);

        List<SportEntity> result = service.findByZbozi(zboziId, offset, count);

        assertNotNull(result);
        assertEquals(1, result.size());
        verify(repository, times(1)).findByZbozi(eq(zboziId), any());
    }
    ...
    void testFindByZbozi_Failure() {
        ...
        when(repository.findByZbozi(eq(zboziId), any())).thenThrow(new RuntimeException("Database error"));
    ...
    void testFindCategoryRoot_Success() throws FinderException {
        ...
        when(repository.findCategoryRoot(any())).thenReturn(expectedEntities);

        List<SportEntity> result = service.findCategoryRoot(offset, count);

        assertNotNull(result);
        assertEquals(1, result.size());
        verify(repository, times(1)).findCategoryRoot(any());
    }

    @Test
    void testFindCategoryRoot_Failure() {
        when(repository.findCategoryRoot(any())).thenThrow(new RuntimeException("Database error"));
